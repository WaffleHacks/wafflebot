...
async def get():
    ...
    for key in Key:
        # Get the value
        result = await getattr(REDIS.kv, key.value)()
...
async def update(key: Key, fields: SettingUpdate):
    ...
    if key in Key.exclude_autogenerated():
        ...
        await getattr(REDIS.kv, f"set_{key.value}")(values)
    ...
    # Update the single field
        ...
        try:
            # Set the new value
            await getattr(REDIS.kv, key.value)(fields.value)
        except ValueError:
